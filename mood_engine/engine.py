...
class Engine:
    ...
    def _get_session(self, user_id, room_id):
        ...
        # 2. DB 로드
        obj = self.DBM.load_session(user_id, room_id)
        self.user_marcov[key] = (obj, current_time)
    ...
    def _cleanup_cache(self, current_time):
        ...
        for k in keys_to_del:
            ...
            self.DBM.save_session(k[0], k[1], obj)

    def _is_uncertain(self, u_preds, g_preds) -> bool:
        """마르코프 체인에 충분한 데이터가 있는지 판단합니다."""
        # 유저 혹은 글로벌 마르코프 중 하나라도 데이터가 있으면 확실하다고 판단
    ...
    def _predict_from_marcov(self, u_preds, g_preds) -> str:
        ...
        # 유저 예측과 글로벌 예측을 결합 (필요 시 여기서 가중치 조절 가능)
    ...
    def query(self, user_id, room_id, message: str) -> str:
        session = self._get_session(user_id, room_id)
        ...
        if should_call_llm:
            # LLM 호출 및 분석
            analyzed_mood = self.PM.query(history_list, message)
            if analyzed_mood:
        ...
        # 4. 이력 업데이트 및 저장 (메모리 캐시만 업데이트, DB 저장은 cleanup 혹은 close 시점에 수행)
    ...
    def close(self):
        ...
        for (user_id, room_id), (obj, _) in self.user_marcov.items():
            self.DBM.save_session(user_id, room_id, obj)
        
        # 2. 글로벌 마르코프 저장
        self.DBM.save_global(self.global_marcov)
